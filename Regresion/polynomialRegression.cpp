...
int main(void) {
	...
	cout << "Ingresar la cantidad de puntos (x,y) de la tabla: ";
	cin >> rows;
	if (rows < 2 || rows > MAXROWS) {
		cout << "Cantidad de puntos invalida (debe ser entre 2 y " << MAXROWS << ")." << endl;
		return 1;
	}
	cout << "Ingresar los " << rows << " puntos (formato por punto: x y):" << endl;
